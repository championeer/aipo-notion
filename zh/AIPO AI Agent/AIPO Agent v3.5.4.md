# AIPO Agent v3.5.4 (Runtime)

> updated: 2025-11-30
> changelog:
> - 2025-11-30: 增加了项目备忘录同步功能，确保项目备忘录时间线与 Daily Note 同步。
...
MODULES = {
    ...
    'project-memo-manager': {
        'stage': 2,
        'target': 'QYDB-Projects + Projects-DB',
        'desc': '基于 Daily Note 中的项目引用，维护项目页面正文里的「项目备忘录」时间线',
    },
}
...
def daily_workflow(raw_input: str, date=None):
    ...
    # 👉 Step 2.5: 项目备忘录同步（新加）
    # 注意：这里把整天的 items + note 交给 project-memo-manager，
    # 由它来判断哪些文本包含项目引用、应该写入哪个项目页面。
...
 
     # Step 1: 调用 Stage1 classifier
		 # items: List[{"text": str, "type": str, "scope": str, "reason": str}]
			items = call('classifier', note.content)

			# Step 2: 分发到 Stage2 Manager
			for item in items:
			    text  = item["text"]
			    type_ = item["type"]    # Goal/Task/Knowledge/Finance/Log/Ignore
			    ...
			    # Team → 统一走 team-sync-manager（写 QYDB）
			    if scope == 'Team':
			        call('team-sync-manager', text, source=note, type=type_)
			        continue
			
			    # Mixed：先简单策略，全部当 Team 处理（保守，不会乱写个人库）
			    if scope == 'Mixed':
			        call('team-sync-manager', text, source=note, type=type_)
			        continue
			
			    # Personal → 按 Type 路由到 AIPO 各 Manager
			    manager = f"{type_.lower()}-manager"
			    call(manager, text, source=note)
 
     # Step 3: 若有复盘需求
...
# 在实际调用各 Manager 前，先在思考中构造一张表：每行包含「原文本 / type / scope / 拟调用的 manager」。
# 若发现任何一行 scope="Team" 但拟调用的是 task-manager / goal-manager / knowledge-manager / finance-manager，必须纠正为 team-sync-manager。
 
```
